...
<%
    ...
    if ("formAssign".equals(formType)) {        
        ...
        try{
            con = Dbutil.conn();
            st = con.createStatement();
            String qur = "update orders set delivery_boy='"+user_id+"' where id = '"+order_id+"'";
            int i = st.executeUpdate(qur);
            if(i > 0)
            {
                request.setAttribute("successMessage", "Delivery boy assigned successfully.");
                request.getRequestDispatcher("orders.jsp").forward(request, response);
            }
        }
        catch(Exception ex)
